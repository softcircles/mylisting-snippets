...
class Expiring_Listings_User_Notification extends Base_Notification {
	...
	public function get_message() {
		...
		$plans_page = c27()->get_setting( 'general_add_listing_page' );
			...
			if ( $listing->type->settings['packages']['enabled'] === false ) {
				$template
					->add_paragraph( sprintf(
						_x( 'Your Listing <strong>%s</strong> will expire on <strong>%s</strong>', 'Notifications', 'my-listing' ),
				...
				$template
					...
					->add_button( _x( 'View listing', 'Notifications', 'my-listing' ), esc_url( $listing->get_link() ) )
					->add_button( _x( 'Renew listing', 'Notifications', 'my-listing' ), esc_url( add_query_arg( ['action' => 'relist', 'listing' => $listing->get_id() ], $plans_page ) ) )
					->add_thematic_break();
			}
		}
