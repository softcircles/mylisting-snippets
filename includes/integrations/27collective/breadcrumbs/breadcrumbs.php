 ...
 * @version 0.1.1
 */
class CASE27_Integrations_Breadcrumbs {
    /**
    ...
    public function generate() {
        ...
        if ( is_singular() || ( is_archive() && ! is_post_type_archive() ) || is_search() || $this->options['show_pagenum'] ) {
            if ( $post_type_link = get_post_type_archive_link( $post_type ) ) {
                ...
                if ( $post_type == 'product' ) {
                    $post_type_label = esc_html__( 'Shop', 'my-listing' );
                }
        ...
        if ( is_singular() ) { // Posts, (Sub)Pages, Attachments and Custom Post Types
            ...
            } elseif ( is_post_type_archive( 'product' ) && ! is_paged() && ($post_type_object = get_post_type_object( $post_type )) ) {
                $post_type_label = esc_html__( 'shop', 'my-listing' );
                $this->breadcrumb["archive_product"] = $this->template( $post_type_label, 'current' );
