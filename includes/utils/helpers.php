...
class Helpers {
	...
	public static function get_video_embed_details( $url ) {
		...
		preg_match_all('%(?:youtube(?:-nocookie)?.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu.be/)([^"&?/ ]{11})|(\?t=|&t=)(\d+)%i', $url, $matches);
		if ( isset( $matches[1][0] ) ) {
			$start_time = ( ! empty($matches[3][1])) ? "?start=".$matches[3][1] : '';
		    return ['url' => "https://www.youtube.com/embed/{$matches[1][0]}{$start_time}", 'type' => 'external', 'service' => 'youtube', 'video_id' => $matches[1][0]];
		}
