...
foreach ( (array) $layout['cover_details'] as $detail ) {
    ...
    if ( ! empty( $detail['content'] ) || in_array( $detail['content'], [ 0, '0', 0.0 ], true ) ) {
        if ( ! empty( $detail['suffix'] ) ) {
            $detail['content'] .= sprintf( '<span class="suffix">%s</span>%s', $detail['suffix'], esc_html__( 'Call For Price', 'my-listing' ) );
        } else {
            $detail['content'] = esc_html__( 'Call For Price', 'my-liting' );
        }
